...
def request_reward(reward_id: str, reward_description: str, reward_price: int) -> None:
    user_reserved_bonus = get_value(
        ...
        field_name="user_reserved_bonuses",
    )
    user_free_bonuses = get_value(
        collection_name="users",
        document_name=st.session_state.username,
    ...
    user_reward_id = add_new_document(
        collection_name="user_reward", document_data=new_user_reward_record
    )
    ...
    update_document(
        collection_name="users",
        document_id=st.session_state.username,
        document_data=updated_user_data,
    )
...
def close_user_challenge(id: str) -> None:
    if st.session_state["panned_finish_" + id] >= datetime.now().date():
        ...
        challenge_reward = get_value(
            collection_name="challenges", document_name=challenge_id, field_name="challenge_reward"
        )
        current_user_bonus = get_value(
            collection_name="users",
            document_name=st.session_state.username,
            field_name="user_free_bonuses",
        )
    ...
    update_document(
        collection_name="user_challenge", document_id=id, document_data=updated_user_challenge_data
    )
    refresh_user_data()
...
def show_user_page() -> None:
    ...
    if selected == "Мои бонусы":
        ...
        with st.container(height=300, border=True):
            ...
            with col1:
                st.header(
                    st.session_state.user_data["user_name"], anchor=None, help=None, divider=False
                )
                st.markdown(
                    f"**Позиция:** {st.session_state.user_data['user_position']}", help=None
                )
                st.markdown(
                    f"**Доступные бонусы:** {st.session_state.user_data['user_free_bonuses']}",
                    help=None,
                )
                st.markdown(
                    f"**Бонусы в резерве:** {st.session_state.user_data['user_reserved_bonuses']}",
                    help=None,
                )
            with col2:
                chart_options = draw_bonus_chart(
                    _free_bonus=st.session_state.user_data["user_free_bonuses"],
        ...
        with st.expander("Выбрать награду", expanded=False):
            ...
            if reward_to_get is not None:
                reward_id = rewards_df[rewards_df["reward_description"] == reward_to_get][
                    "id"
                ].values[0]
        ...
        with st.expander("Выбрать новое задание", expanded=False):
            ...
            with col1:
                ...
                challenge_id = st.session_state.challenge_df[
                    st.session_state.challenge_df["challenge_description"] == challenge_to_assign
                ]["id"].values[0]
        ...
        with st.container():
            user_challenges_new = get_user_challenges(
                user_id=st.session_state.user_id, challenge_status="new"
            )
            new_challenges_ids = []
                ...
                with st.form(f"challenge_form_{challenge.id}"):
                    ...
                    with col2:
                        ...
                        st.date_input(
                            ...
                            value=end_date,
                            format="DD/MM/YYYY",
                            key=f"panned_finish_{challenge.id}",
                            disabled=True,
                        )
            ...
            with st.container():
                user_challenges_ongoing = get_user_challenges(
                    user_id=st.session_state.user_id, challenge_status="ongoing"
                )
        ...
        st.markdown(
            body="Для того, чтобы подключить уведомления, перейдите по ссылке в Telegram бот."
        )
